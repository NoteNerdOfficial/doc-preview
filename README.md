...
Preview PowerPoint and Word documents inside Obsidian — no need to open the real app.
...
- Opens `.pptx` and `.docx` files directly in an Obsidian tab instead of launching PowerPoint/Word
...
- Custom PDF viewer: zoom in/out, fit-to-page, rotate, a toggleable thumbnail rail, keyboard arrow-key navigation, and a download button
...
1. Clicking a `.pptx`/`.docx` file opens Doc Preview's own view instead of the OS default app (via `registerExtensions`).
